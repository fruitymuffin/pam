...
public:
    ...
    bool Open()
    {
        ...
        PvResult lResult = mDevice->GetParameters()->GetIntegerValue( "SourceIDValue", lSourceChannel );
        if ( !lResult.IsOK() )
        {
            // Older device ? try SourceStreamChannel (deprecated).
            lResult = mDevice->GetParameters()->GetIntegerValue( "SourceStreamChannel", lSourceChannel );
        }

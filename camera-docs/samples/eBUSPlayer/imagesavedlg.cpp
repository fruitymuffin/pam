...
///
/// \brief Dynamically add the file type to the combo box with set the index
///

void ImageSaveDlg::AddToFormatComboBox( QString aFileFormatString,  ImageSaving::Format aFormat )
{
    if ( nullptr != mFormatCombo )
    {
        mFormatCombo->addItem( aFileFormatString );
        mFormatComboToIndex[ aFormat ] = mFormatCombo->count() - 1;
    }
}
...
QGroupBox *ImageSaveDlg::CreateFormatBox()
{
    ...
    AddToFormatComboBox( "Device independent bitmap (BMP)", ImageSaving::FormatBmp ); 
    AddToFormatComboBox( "Tagged Image File Format (TIFF)", ImageSaving::FormatTiff );
    AddToFormatComboBox( "Portable Network Graphics (PNG)", ImageSaving::FormatPNG );
    AddToFormatComboBox( "Raw binary data", ImageSaving::FormatRaw );

    if ( mImageSaving->IsMp4Supported() )
    {
        AddToFormatComboBox( "H.264 video in MP4 container", ImageSaving::FormatMp4 );
    }
    AddToFormatComboBox( "Pleora Decompressed (BIN)", ImageSaving::FormatPleoraDecompressed );

    mH264bitrate = new QLabel( tr( "Target encoding bitrate" ) );
    mH264bitrateValue = new QLineEdit( tr( "2000000" ) );
    mH264bitrateUnit = new QLabel( tr( "bits/s" ) );

    QObject::connect( mFormatCombo, SIGNAL( currentIndexChanged( int ) ), this, SLOT( OnFormatChanged( int ) ) );

    QHBoxLayout *lBitRate = new QHBoxLayout;
    lBitRate->addWidget( mH264bitrate );
    lBitRate->addWidget( mH264bitrateValue );
    lBitRate->addWidget( mH264bitrateUnit );

	QVBoxLayout *lLayout = new QVBoxLayout;
	lLayout->addWidget(mFormatCombo);
    lLayout->addLayout(lBitRate);
...
///
/// \brief Format Change handler
///

void ImageSaveDlg::OnFormatChanged( int aIndex )
{
    if ( aIndex == 3 )
    {
        EnableInterface();
...
void ImageSaveDlg::EnableInterface()
{
    ...
    int lMP4CurIndex = -1;
    std::map<ImageSaving::Format, int>::iterator lItr = mFormatComboToIndex.find( ImageSaving::FormatMp4 );
    if ( lItr != mFormatComboToIndex.end() )
    {
        lMP4CurIndex = lItr->second;
    }
    mH264bitrateValue->setEnabled( lEnabled && mFormatCombo->currentIndex() == lMP4CurIndex );
}
...
{
    ImageSaving::Format lFormat = mImageSaving->GetFormat();
    std::map<ImageSaving::Format, int>::iterator lIt;
    for ( lIt = mFormatComboToIndex.begin(); lIt != mFormatComboToIndex.end(); lIt++ )
    {
        if ( lIt->first == lFormat )
        {
            mFormatCombo->setCurrentIndex( lIt->second );
            break;
        }
    }

    QVariant lAvgBitrate( mImageSaving->GetAvgBitrate() );
    mH264bitrateValue->setText( lAvgBitrate.toString() );
...
bool ImageSaveDlg::FromDialog()
{
    ...
    mImageSaving->SetPath( lPathText.toUtf8().constData() );
    mImageSaving->SetEnabled( mSaveEnabledCheck->isChecked() );

    int lIndex = mFormatCombo->currentIndex();
    std::map<ImageSaving::Format, int>::iterator lIt;
    for ( lIt = mFormatComboToIndex.begin(); lIt != mFormatComboToIndex.end(); lIt++ )
    {
        if ( lIt->second == lIndex )
        {
            mImageSaving->SetFormat( lIt->first );
            break;
        }
    }

    QString lBitRateString = mH264bitrateValue->text();
    mImageSaving->SetAvgBitrate( lBitRateString.toUInt() );
